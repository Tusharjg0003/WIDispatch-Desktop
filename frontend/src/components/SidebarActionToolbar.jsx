...
export default function SidebarActionToolbar({
    ...
    return (
      <div className="sidebar-action-toolbar">
        <div className="sidebar-action-toolbar__buttons">
          ...
          >
            <Trash2 size={11} /> {deleting ? "Deleting…" : `Delete${selectedCount ? ` (${selectedCount})` : ""}`}
          </button>
          <button
            type="button"
  ...
  const buttonCount = 1 + (showFilter ? 1 : 0) + (showSort ? 1 : 0) + (showDelete ? 1 : 0)
    + (onOverview ? 1 : 0) + extraActions.length;

  return (
    <div className="sidebar-action-toolbar">
      <div className="sidebar-content__actions">
        <button type="button" className="sidebar-content__action-btn" onClick={onCreate}>
          <Plus size={11} /> {createTitle}
        </button>
      </div>
      ...
      >
        ...
        {extraActions.map(({ title, icon: Icon, onClick, active }, i) => (
          <button
            key={title || i}
            type="button"
